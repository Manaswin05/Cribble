  ...
  return (
    <div className={`flex flex-col h-full overflow-hidden bg-[#f3f0ef] text-[#2f2e2e] transition-all duration-300`}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside
          className="flex-shrink-0 flex flex-col bg-[#16213e] border-l border-white/5 overflow-hidden transition-[width] duration-300 ease-in-out relative text-white"
        ...
        >
          <button
            ...
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-full z-50 flex flex-col items-center justify-center w-5 h-16 bg-[#16213e] border border-white/10 border-r-0 rounded-l-lg text-white/30 hover:text-white/80 hover:bg-[#0f3460] transition-all shadow-xl"
          ...
          <div className="flex flex-shrink-0 border-b border-white/5 min-w-[320px]">
            <button onClick={() => setRightPanelTab('properties')}
              className={`flex-1 flex items-center justify-center gap-2 px-3 py-3 text-xs font-bold transition-all ${rightPanelTab === 'properties' ? 'text-white border-b-2 border-[#6a37d4]' : 'text-white/30 hover:text-white/60'}`}>
              <Layers size={13} /> Properties
            </button>
            <button onClick={() => setRightPanelTab('chat')}
              className={`flex-1 flex items-center justify-center gap-2 px-3 py-3 text-xs font-bold transition-all ${rightPanelTab === 'chat' ? 'text-white border-b-2 border-[#6a37d4]' : 'text-white/30 hover:text-white/60'}`}>
              <Sparkles size={13} /> AI Chat
              {IS_REAL_AI && <span className="w-1.5 h-1.5 bg-green-400 rounded-full" />}
            </button>
            <button onClick={() => setIsRightSidebarCollapsed(v => !v)} className="px-3 text-white/20 hover:text-white/60 transition-colors" title="Collapse">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6 scrollbar-hide min-w-[320px]">
              <section>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-sm text-white/80">Properties</h3>
                  <span className="text-[10px] font-bold text-violet-400 bg-violet-500/10 border border-violet-500/20 px-2 py-0.5 rounded">TEXT LAYER</span>
                </div>
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {[['Width', '480'], ['Height', '120']].map(([label, val]) => (
                    <div key={label} className="space-y-1">
                      <label className="text-[10px] font-bold text-white/30 uppercase">{label}</label>
                      <div className="flex items-center px-3 py-2 bg-white/5 border border-white/8 rounded-lg">
                        <input className="w-full bg-transparent border-none p-0 text-xs font-bold text-white/70 focus:outline-none" type="text" defaultValue={val} />
                ...
                <div className="space-y-3">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-white/30 uppercase">Font Family</label>
                    <div className="flex items-center justify-between px-3 py-2 bg-white/5 border border-white/8 rounded-lg cursor-pointer hover:bg-white/8">
                      <span className="text-sm font-serif font-bold text-white/70">Noto Serif</span>
              ...
              <section className="border-t border-white/5 pt-5">
                <p className="text-[10px] uppercase tracking-widest text-white/30 font-bold mb-4">Appearance</p>
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-white/60">Opacity</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-white/60">Fill</span>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-mono text-white/30">#2F2E2E</span>
                      <div className="w-6 h-6 rounded-md bg-[#2f2e2e] border border-white/10 cursor-pointer shadow-sm" />
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section className="border-t border-white/5 pt-5">
                <p className="text-[10px] uppercase tracking-widest text-white/30 font-bold mb-4">AI Generator</p>
                <div className="space-y-2">
                  <textarea
                    value={aiPromptInput}
                    ...
                    rows={3}
                    className="w-full text-[12px] bg-white/5 border border-white/10 rounded-xl p-3 resize-none focus:outline-none focus:ring-1 focus:ring-[#6a37d4]/50 placeholder:text-white/20 text-white/80 leading-relaxed shadow-sm"
                  />
          ...
          {rightPanelTab === 'chat' && (
            <div className="flex-1 flex flex-col overflow-hidden min-w-[320px]">
                ...
                {chatMessages.map((msg) => (
                  <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}
                    ...
                    <div className={`max-w-[85%] ${msg.sender === 'user' ? 'ml-auto' : ''}`}>
                      ...
                      <span className={`text-[9px] text-white/20 mt-0.5 block ${msg.sender === 'user' ? 'text-right' : ''}`}>{msg.time}</span>
                    </div>
                  </motion.div>
                ))}
                {isBotTyping && (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex gap-2">
                    ...
                    <div className="bg-white/5 border border-white/8 px-4 py-3 rounded-2xl rounded-bl-md">
              ...
              <div className="flex gap-1.5 px-4 py-2 overflow-x-auto scrollbar-hide flex-shrink-0 border-t border-white/5 bg-black/20">
                {[
                  ...
                  <button key={chip.label}
                    ...
                    className="flex items-center gap-1 flex-shrink-0 px-2.5 py-1.5 text-[10px] font-bold bg-white/5 border border-white/10 rounded-full hover:border-violet-500/40 hover:text-violet-300 text-white/40 transition-all whitespace-nowrap"
                  >
                    <chip.icon size={10} />
                    {chip.label}
                  </button>
              ...
              <div className="p-3 border-t border-white/5 flex-shrink-0 bg-[#16213e]">
                <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-1.5 focus-within:border-violet-500/40 transition-all">
                  <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)}
                    ...
                    className="flex-1 bg-transparent border-none focus:outline-none text-xs font-medium placeholder:text-white/20 text-white/80 py-1.5"
                  />
